...
trait AuthTokensComponent { this: DriverComponent with UsersComponent =>
    import driver.api._
    ...
    def generate(forWho:Int, expires_in : Int = 3600)(implicit db : slick.jdbc.JdbcBackend.Database) : (String) = {
        ...
        db.run(authTokens += Token(token, new Timestamp(Calendar.getInstance().getTime().getTime()), true, forWho))
        token
    }
}
